...
class NodeId(object):
    ...
    NodeIdType: NodeIdType = field(default=NodeIdType.NumericNodeId, compare=False, hash=False)
        ...
        if self.NodeIdType is None:
            if isinstance(self.Identifier, int):
                if self.Identifier < 255 and self.NamespaceIndex == 0:
                    ...
                    self.NodeIdType = NodeIdType.FourByte
                else:
    ...
    def check_identifier_type_compatibility(self):
        ...
        for identifier, valid_node_types in valid_type_combinations:
            if isinstance(self.Identifier, identifier) and self.NodeIdType in valid_node_types:
                return
        raise UaError(
            f"NodeId of type {self.NodeIdType} has an incompatible identifier {self.Identifier} of type {type(self.Identifier)}"
        )
...
class QualifiedName:
    ...
    def __pos_init__(self):
        ...
        if not isinstance(self.NamespaceIndex, int) or not isinstance(self.Name, str):
            raise ValueError(f"QualifiedName constructore args are wrong, {self}")
...
class LocalizedText:
    ...
    Encoding: Byte = field(default=0, repr=False)
...
class Variant:
    ...
    def __str__(self):
        # FIXME: instead of that make some members not visible in datalass config
...
class DataValue:
    ...
    Encoding: Byte = field(defualt=0, repr=False)
    Value: Optional(Variant) = None
    StatusCode: Optional(StatusCode) = StatusCode()
